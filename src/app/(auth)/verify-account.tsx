  ...
  return (
    <ImageBackground source={images.bgAuth} resizeMode="cover" className="h-full">
      ...
      <View className="justify-center mt-10 p-6 rounded-xl pt-48 pb-12">
        <Text className="text-center text-3xl font-normal text-primary font-semibold mt-10">Xác thực</Text>
        <Text className="mt-2 text-center mb-4 text-gray-500">
          ...
          {Array(6)
            ...
            .map((_, index) => (
              <TextInput
                ...
                className="w-[50px] h-[60px] rounded-xl border border-[#919EAB52] text-center text-[22px] text-primary"
        ...
        <CustomButton
          ...
          onPress={onSubmit}
          containerStyle={`w-full mt-7 mb-2 ${isLoading ? 'bg-[#831FE0]' : 'bg-[#EC38BC]'} min-h-12`}
        ...
        >
          {isLoading ? (
            ...
            <Text className="text-red-600 font-semibold">Gửi lại</Text>
          )}
        </TouchableOpacity>
        ...
        >
          ...
          <Text className="text-primary text-sm font-semibold">Quay lại đăng nhập</Text>
        </TouchableOpacity>
      </View>
    </ImageBackground>
  )
