...
export default function SearchScreen() {
  ...
  const { searchQuery, topChartsQuery, recentTracksQuery, popularArtistsQuery } = useHome()
  ...
  const renderDefaultContent = () => {
    return (
      <>
        <TitleHome text="Bài hát gần đây" color="#EC38BC" />
        <FlatList
          className="gap-2 mt-4 mb-8"
          showsVerticalScrollIndicator={false}
          contentContainerStyle={{ gap: 20 }}
          data={recentTracksQuery.data?.data || []}
          renderItem={({ item }) => (
            <TouchableOpacity
              onPress={() => {
                router.push({
        ...
        />
        <TitleHome text="Nghệ sĩ phổ biến" color="#B1FF4D" />
        <FlatList
          className="gap-2 mt-4 mb-8"
          showsVerticalScrollIndicator={false}
          contentContainerStyle={{ gap: 20 }}
