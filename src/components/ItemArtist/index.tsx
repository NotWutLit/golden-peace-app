  ...
  return (
    ...
    >
      <Image
        ...
        resizeMode="cover"
      />
      <Text className="mt-2 w-32 text-center text-white" numberOfLines={1}>
        {data?.name || 'Jack – J97'}
      </Text>
      <Text className="text-[#919EAB] w-32 text-center text-xs" numberOfLines={1}>
        {data?.playCount} người theo dõi
      </Text>
      <View className="items-center">
        {data?.isFollowed ? (
          <TouchableOpacity
            className="flex-row gap-2 border border-[#B1FF4D8F] p-2 rounded-2xl items-center"
            onPress={handleUnFollow}
            disabled={unfollowArtistMutation.isPending}
          >
            <IconTick />
            <Text className="text-[#B1FF4D]">
          ...
          >
            <Text className="text-white">
              {followArtistMutation.isPending ? 'Đang xử lý...' : 'Theo dõi'}
            </Text>
          </TouchableOpacity>
        )}
      </View>
