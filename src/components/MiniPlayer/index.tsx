...
export default function MiniPlayer() {
  ...
  const openFullPlayer = () => {
    ...
    if (global.globalLocalTrackId && global.globalLibraryType) {
      router.push({
        pathname: '/(children)/play-music',
        params: { 
          localTrackId: global.globalLocalTrackId,
          libraryType: global.globalLibraryType
        }
      })
    } else if (global.globalCurrentSongId) {
      // Backend track
      router.push({
        pathname: '/(children)/play-music',
  ...
  return (
    <Animated.View
      ...
      className="absolute bottom-36 left-0 right-0 bg-[#212B36] mx-14 rounded-3xl p-3 flex-row items-center justify-between shadow-lg z-50"
    >
      {/* Song Info */}
      <TouchableOpacity
